...
router.post("/savepayslips", async (req, res) => {
  ...
  router.post('/updateEmp/:id', async (req, res) => {
    ...
    const data = connection.query(query1, [id])
    const query2 = "UPDATE employee SET Name = ?, email = ?, phone = ?, password = ?, role = ? WHERE empId = ?"
    await connection.query(query2, [req.body.name, email, phone])
  })
